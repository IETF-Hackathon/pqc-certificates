...
_OID_MAPPING_LINE_REGEX = re.compile(r'^\|\s*(?P<name>[^|]+)\s*\|\s*(~~)?(?P<oid>[A-Za-z0-9][A-Za-z0-9.\-]*)\*?(~~)?\s*\|.*$')
...
def main():
    ...
    md_file.new_paragraph(text="<style> table { border-collapse: collapse; width:auto !important; } thead th { position: sticky; top: 0; z-index: 1; background: white; } th, td { border: solid black 1px; padding: 0 1ex; } col { width: auto !important; } </style>")
    ...
    md_file.new_paragraph(text="✅ = passing all verifiers<br>◒ = passing some verifiers<br>⚪︎ = not passing any verifiers<br>Columns represent producers who submitted artifacts. <br>")
    md_file.new_paragraph()
    ...
    for alg_oid, algorithmVerificationResults in avrs_by_alg.items():
        ...
        md_file.new_header(level=1, title=f'{friendly}', header_id=friendly)
